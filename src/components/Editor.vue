...
const init = {
    ...
    toolbar:
        "formats undo redo fontsizeselect fontselect ltr rtl searchreplace media imageUpload | outdent indent aligncenter alignleft alignright alignjustify lineheight underline quicklink h2 h3 blockquote numlist bullist table removeformat forecolor backcolor bold italic strikethrough hr link preview fullscreen help image ",
    ...
    file_picker_types: "file | image | media", //指定所需的文件选择器类型。目前存在三种有效类型：file、image和media。
    ...
    setup: (editor) => {
        ...
        editor.ui.registry.addButton("imageUpload", {
            ...
            icon: "image",
            onAction() {
                SelectImageRef.value.openDialog((data) => {
                    data.forEach(url => {
...
<template>
    ...
    <SelectImage ref="SelectImageRef" :limit="9" :preview="false" />
</template>
